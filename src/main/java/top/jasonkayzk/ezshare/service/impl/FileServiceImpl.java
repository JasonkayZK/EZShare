...
import top.jasonkayzk.ezshare.config.SettingConfig;
import top.jasonkayzk.ezshare.dao.DownloadedDAO;
import top.jasonkayzk.ezshare.dao.FileDAO;
import top.jasonkayzk.ezshare.entity.Category;
import top.jasonkayzk.ezshare.entity.File;
import top.jasonkayzk.ezshare.entity.User;
import top.jasonkayzk.ezshare.model.AuthRecord;
import top.jasonkayzk.ezshare.model.BaseAuthRecord;
import top.jasonkayzk.ezshare.model.FileBasicRecord;
import top.jasonkayzk.ezshare.model.FileRecord;
import top.jasonkayzk.ezshare.modules.constant.ConfigConsts;
import top.jasonkayzk.ezshare.modules.constant.DefaultValues;
import top.jasonkayzk.ezshare.service.IAuthService;
import top.jasonkayzk.ezshare.service.ICategoryService;
import top.jasonkayzk.ezshare.service.IFileService;
...
public class FileServiceImpl implements IFileService {
    ...
    public boolean updateFileInfo(long id, User user, String name, String category, String tag, String description) {
        ...
        if (Checker.isNotNull(file) && file.getIsUpdatable() == 1) {
            ...
            boolean canUpdate = (Checker.isNull(authRecord) ? user.getIsUpdatable() == 1 :
                    authRecord.getIsUpdatable() == 1) && Checker.isNotEmpty(name) && Pattern.compile(EZShareApplication.settings.getStringUseEval(ConfigConsts.FILE_SUFFIX_MATCH_OF_SETTING)).matcher(suffix).matches();
            if (canUpdate) {
    ...
    public String getResource(String visitUrl, HttpServletRequest request) {
        ...
        boolean downloadable = EZShareApplication.settings.getBooleanUseEval(ConfigConsts
    ...
    public boolean upload(int categoryId, String tag, String description, String prefix, MultipartFile multipartFile,
        ...
        if (user.getIsUploadable() == 1) {
            ...
            long maxSize = Formatter.sizeToLong(EZShareApplication.settings.getStringUseEval(ConfigConsts
                    .FILE_MAX_SIZE_OF_SETTING));
            long size = multipartFile.getSize();
            boolean fileExists = localUrlExists(localUrl);
            //检测标签是否合法
            if (EZShareApplication.settings.getBooleanUseEval(ConfigConsts.TAG_REQUIRE_OF_SETTING)) {
                String[] tags = Checker.checkNull(tag).split(ValueConsts.SPACE);
                if (tags.length <= EZShareApplication.settings.getIntegerUseEval(ConfigConsts.TAG_SIZE_OF_SETTING)) {
                    int maxLength = EZShareApplication.settings.getIntegerUseEval(ConfigConsts.TAG_LENGTH_OF_SETTING);
                    for (String t : tags) {
            ...
            //是否可以上传
            boolean canUpload = !multipartFile.isEmpty() && size <= maxSize && Pattern.compile(EZShareApplication
                    .settings.getStringUseEval(ConfigConsts.FILE_SUFFIX_MATCH_OF_SETTING)).matcher(suffix).matches()
                    && (Checker.isNotExists(localUrl) || !fileExists || EZShareApplication.settings.getBooleanUseEval
            ...
            if (canUpload) {
                String visitUrl = getRegularVisitUrl(Checker.isNotEmpty(prefix) && user.getPermission() > 1 ? prefix
                        : EZShareApplication.settings.getStringUseEval(ConfigConsts.CUSTOM_LINK_RULE_OF_SETTING), user,
