...
public class FileSqlProvider {
    ...
    public String getBasicBy(@Param("userId") int userId, @Param("fileId") long fileId, @Param("fileName") String
        ...
        String sql = new SQL() {{
            ...
            ORDER_BY("f." + EZShareApplication.settings.getStringUseEval(ConfigConsts.FILE_ORDER_BY_OF_SETTING));
        }}.toString();
        int size = EZShareApplication.settings.getIntegerUseEval(ConfigConsts.FILE_PAGE_SIZE_OF_SETTING);
    ...
    private String getSqlEnds(int offset, String orderBy, String search) {
        int size = EZShareApplication.settings.getIntegerUseEval(ConfigConsts.FILE_PAGE_SIZE_OF_SETTING);
        return getSearch(search) + " order by " + (Checker.isEmpty(orderBy) ? EZShareApplication.settings
