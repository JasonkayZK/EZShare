package top.jasonkayzk.ezshare.web.controller;

import com.zhazhapan.modules.constant.ValueConsts;
import com.zhazhapan.util.FileExecutor;
import com.zhazhapan.util.NetUtils;
import top.jasonkayzk.ezshare.EZShareApplication;
import top.jasonkayzk.ezshare.annotation.AuthInterceptor;
...
public class ConfigController {
    ...
    public String updateConfig(String config) {
        ...
        if (user.getPermission() > ValueConsts.TWO_INT) {
            EZShareApplication.settings.setJsonObject(config);
            //打包成jar之后无法修改config.json文件
            try {
                FileExecutor.saveFile(NetUtils.urlToString(EZShareApplication.class.getResource(DefaultValues
                        .SETTING_PATH)), EZShareApplication.settings.toString());
            } catch (IOException e) {
    ...
    public String getAll() {
        User user = (User) request.getSession().getAttribute(ValueConsts.USER_STRING);
        if (user.getPermission() > ValueConsts.TWO_INT) {
            return EZShareApplication.settings.toString();
        } else {
            return "{\"message\":\"permission denied\"}";
        }
    }
