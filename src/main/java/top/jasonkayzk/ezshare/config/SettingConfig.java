package top.jasonkayzk.ezshare.config;
...
import top.jasonkayzk.ezshare.util.CommonUtils;
...
public class SettingConfig {
    ...
    public static int[] getAuth(String jsonPath) {
        ...
        for (int i = 0; i < ConfigConsts.AUTH_OF_SETTINGS.length; i++) {
            ...
            auth[i] = EZShareApplication.settings.getBooleanUseEval(key) ? 1 : 0;
        }
        return auth;
    }
    ...
    public static String getUploadStoragePath() {
        ...
        String formatWay = EZShareApplication.settings.getStringUseEval(ConfigConsts.UPLOAD_FORM_OF_SETTING);
    ...
    public static String getStoragePath(String path) {
        ...
        return CommonUtils.checkPath(EZShareApplication.settings.getStringUseEval(path));
    }
