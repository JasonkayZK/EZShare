...
public class TokenConfig {
    ...
    public static String generateToken(String token, int userId) {
        if (Checker.isNotEmpty(token)) {
            EZShareApplication.tokens.remove(token);
        }
    ...
    public static String generateToken(int userId) {
        ...
        EZShareApplication.tokens.put(token, userId);
        saveToken();
        return token;
    }

    public static void saveToken() {
        String tokens = Formatter.mapToJson(EZShareApplication.tokens);
        try {
    ...
    public static void removeTokenByValue(int userId) {
        if (userId > 0) {
            String removeKey = "";
            for (String key : EZShareApplication.tokens.keySet()) {
                if (EZShareApplication.tokens.get(key) == userId) {
                    removeKey = key;
                    break;
                }
            }
            if (Checker.isNotEmpty(removeKey)) {
                EZShareApplication.tokens.remove(removeKey);
